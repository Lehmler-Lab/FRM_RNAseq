...
for (i in names(my_list2)) {
    print(i)
  x<- my_list2[[i]][order(my_list2[[i]]$log2FoldChange, decreasing = TRUE),] #reorder the genes based on significance
gene_list<-x$log2FoldChange
names(gene_list)<-x$entrez

plot_list_enrich[[i]] <- gene_list
}
...
STN_groups <- compareCluster(geneClusters=plot_list_enrich,  fun = "gseGO", OrgDb=org.Mm.eg.db, pvalueCutoff = 0.05, pAdjustMethod = "none")
```


```{r}
#tiff(file = 'enrichment_figures/grouped_gene_enrichment.tiff', units="in", width=8, height=5, res=1000)
dot36<-dotplot(STN_groups,showCategory=2, split=".sign") +
